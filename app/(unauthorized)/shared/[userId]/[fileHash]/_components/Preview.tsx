...
const SharedFilePreview: React.FC = () => {
    const { userId, fileHash } = useParams<{
        userId: string;
        fileHash: string;
    }>();
    ...
    }, [currentPath, fileHash, userId]);

    useEffect(() => {
        fetchFileInfo();
    }, [userId, fileHash, currentPath, fetchFileInfo]);
            ...
            <div className="p-4 md:p-5">
                <div className="flex flex-col items-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-4">
                        Shared on:{' '}
                        {new Date(fileInfo.sharedAt).toLocaleString()}
                    </p>
                    {!fileInfo.isDirectory && (
                        <button
                            type="button"
                            ...
                            onClick={() =>
                                handleDownload(fileInfo.originalName)
                            }
                        >
