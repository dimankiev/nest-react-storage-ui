...
import { BiPencil, BiFolder, BiFile, BiShare, BiTrash, BiDownload } from "react-icons/bi";
    ...
    ({
        ...
        >
            <div className="itemName inline-flex items-center gap-x-3.5">
                <div>
                    {item.isDirectory ? <BiFolder/> : <BiFile/>}
                </div>
                <div
                    className={`itemName__text inline-flex gap-x-1.5 items-center` + ((item.name === '..') && ' goBack' || '')}
                >
                    {item.name}
                    {item.name !== '..' && (
                        <div
